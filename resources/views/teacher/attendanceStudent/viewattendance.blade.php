        ...
        <div class="card border-0">
            <div class="card-body pt-4">
                <h4 class="mb-4 d-flex justify-content-between">
                    <div class="fw-bold">Select Class and Section </div><select style="width: 250px" name=""
                        id="getClassAndSectionId" class="form-select">
                        @if (!@empty($teacher_details->teacherclassmapping))
                            @foreach ($teacher_details->teacherclassmapping as $item)
                                <option value="{{ $item->class_id }}" data-section="{{ $item->section_id }}">
                                    {{ @$item->teacherClass->class_name }} ( {{ @$item->teacherSection->section_name }} )
                                </option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="row my-3">
                    ...
                    <div class="col-4 mb-2" id="showSelectDate">
                        ...
                        <input class="form-control" type="date" id="dateAttendance" value="{{ date('Y-m-d') }}"
                            name="dateAttendance">
                    </div>
                    ...
                    <div class="col-md-12 col-sm-12 text-end mt-2">
                        <button onclick="viewAttendance()" type="submit" class="btn btn-primary me-2">View
                            Attendance</button>
                    </div>
                </div>
            </div>
        ...
        <div class="card border-0">
            <div class="card-body pt-4">
                <div class="d-flex justify-content-between mb-4">
                    ...
                    <button id="exportExcel" class="btn btn-secondary btn-sm"><i class="bi bi-file-excel"></i> Export to
                        Excel</button>
                </div>
                            ...
                            },
                            {
                                ...
                                render: function(data) {
                                    if (data.date_taken == "{{ date('Y-m-d') }}") {
                                        return '<a class="btn btn-danger btn-sm rounded-pill show_confirm" href="javascript:void(0)" onclick="deleteStudentAttendance(' +

                ...
                <form id="formData" action="{{ route('studentFeeSettings.store') }}" method="post">
                    ...
                    <div class="row my-3">
                        ...
                        <div class="col-4 mb-2">
                            <label for="" class="form-label">Select Session<span
                        ...
                        <div class="col-4 mb-2">
                            <label for="" class="form-label">Select Section <span
                                    class="text-danger">*</span></label>
                            <select name="section_id" id="section_id" class="form-select"
                                onchange="GetSessionPaymentSettingsData(); getStudent()" required>
                        ...
                        <div class="col-6 mb-4">
                            <label for="" class="form-label">Select Student<span
                                    class="text-danger">*</span></label>
                            <select name="user_id" id="student_id_bind" class="form-select js-example-basic-single"
                                onchange="GetSessionPaymentSettingsData();" required>
                                                ...
                                                <div class="col-6">
                                                    <div class="input-group mb-3">
                                                        ...
                                                        <input type="number" min="0"
                                                            name="charges_amount[{{ @$item->id }}]" class="form-control"
                                                            @if (@$item->fixed_amount != null) value="{{ @$item->fixed_amount }}"
                                                        @else
                                                            value="{{ old('charges_amount.' . @$item->id) }}" @endif
                                                            id="charges_amount_{{ @$item->id }}">
                                                    </div>
                                                    @if ($errors->has('charges_amount.' . $item->id))
                                                        <span
                                                            class="text-danger">{{ $errors->first('charges_amount.' . $item->id) }}</span>
                                                    @endif
                                                </div>
                                            </div>
                                            ...
                                            @foreach (GetallMonths() as $month)
                                                <div class="form-check form-check-inline">
                                                    ...
                                                    <small class="form-check-label"
                                                        for="{{ $month }}_{{ $item->id }}">{{ $month }}</small>
                                                </div>
                                            @endforeach

                                            @if ($errors->has('months_validation.' . $item->id))
                                                <span
                                                    class="text-danger">{{ $errors->first('months_validation.' . $item->id) }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                @endif
                        ...
                        <div class="col-12 text-end">
                            <button type="submit" id="add_payment_settings" class="btn btn-success">Add Student Fee
                                Settings</button>
                            <button type="submit" id="update_payment_settings" class="btn btn-primary">Update Student Fee
                                Settings</button>
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, payemtsetting) {
                            ...
                            var paymentHtml = result.map(function(component) {
                                var html =
                                    '<div style="background:#FFF3CD; padding:10px; border-bottom:2px solid #ffffff">';
                                html += '<strong>' + component.name + '</strong><br>';
                                html += '<span>' + component.amount + '</span><br>';
                                html += '<strong>Months:</strong> ' + (component.months.length ?
                                    component.months.join(', ') : '-----');
                                html += '</div>'
                                return html;
                            }).join('');
                            ...
                            rowData.push(
                                '<a class="btn btn-primary btn-sm me-2 mb-2" href="javascript:void(0)" onclick=\'EditPaymentSettings(' +
                                JSON.stringify(payemtsetting) +
                                ')\'><i class="bi bi-pencil-square"></i></a>' +
                                '<a class="btn btn-danger btn-sm show_confirm" href="javascript:void(0)" onclick="deletePaymentSettings(\'' +
                                payemtsetting.id + '\')"><i class="bi bi-trash"></i></a>'
                            );

...
<div class="pagetitle">
    ...
    <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('student.entrollmentHistory') }}">Transfer & Promote History</a></li>
        ...
        <div class="card-body pt-4">
            ...
            <div class="row my-4">
                <div class="col-4 mb-2">
                    <label for="" class="form-label">Current Class - Section<span class="text-danger">*</span></label>
                    <select name="class_id_section_id" id="class_id_section_id" class="form-select" onchange="GetClassWiseSection(value)">
                 ...
                 <div class="col-4 mb-2">
                    <label for="" class="form-label">Transfer Class Section<span class="text-danger">*</span></label>
                ...
                <table class="table-bordered table table-striped" id="DataTables">
                        ...
                        <tr class="table-primary">
                            <th class="text-center" style="width: 20px">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" value="all_student_checked" onchange="toggleAllUsers(this)" id="all_student_checked">
                                </div>
                            </th>
                            <th>NO.</th>
                            <th>Name</th>
                    ...
                    <tbody id="studentTableBody">
                        <tr class="text-center" id="ShowHideNoStudent">
                            <td colspan="3"  class="fw-bold">No Student's Found</td></tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="card-body pt-4">
            ...
            <div class="row my-4">
                ...
                <div class="col-3 mb-2">
                    ...
                    <select name="previous_class_id_section_id" id="previous_class_id_section_id" class="form-select" onchange="getAllStudent('deactive_session', 'previous_session_id', 'previous_class_id_section_id', 'priviousSessionStudent')">
                        <option value="">--Select Class--</option>
                        @if (!@empty(@$newSections))
                            @foreach (@$newSections as $index=>$item)
                                <option value="{{@$item['id']}}_{{@$item['class_id']}}">{{@$item['class_section']}}</option>
                ...
                <div class="col-3 mb-2">
                    <label for="" class="form-label">Promote Class - Section<span class="text-danger">*</span></label>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.length < 2){
                        $('#class_id_wise_section_id').append('<option value="null" selected">--No Data Found--</option>');
                        $('#class_id_wise_section_id').prop('disabled', true);
                    }else{
                        ...
                        $.each(response, function(index, item) {
                            ...
                            if(item.id+'_'+item.class_id != section_class){
                                $('#class_id_wise_section_id').append('<option value="' + item.id + '_' + item.class_id +'">' + item.class_section + ') </option>');
                            }
                        });
                    }
                },
                ...
                success: function(response) {
                    ...
                    if(tableBody.attr('id') == 'priviousSessionStudent'){
                        ...
                        if(response.length > 0){
                            ...
                            tableBody.append('<tr><td colspan="3" class="fw-bold text-center">No students found.</td></tr>');
                        }
                    }else{
                        tableBody.empty(); 
                        if(response.length > 0){
                ...
                if(user_id && session_id && class_id && section_id){
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            getAllStudent('deactive_session', 'previous_session_id', 'previous_class_id_section_id', 'priviousSessionStudent');
                            Notiflix.Notify.Success(response[1]);
                        },error: function(xhr, status, error) {
                            Notiflix.Notify.Failure(response[1]);
                        }

...
@section('content')
                ...
                <form action="" method="post">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            ...
                            <input type="text" id="admission_number" placeholder="Search by admission number"
                                class="form-control" name="admission_number">
                        </div>
                        <div class="col-4">
                            <label for="student_name" class="form-label">Student</label>
                            <input type="text" id="student_name" placeholder="Search by student name"
                                class="form-control" name="student_name">
                        </div>
                        <div class="col-4">
                            <label for="mobile_no" class="form-label">Mobile No</label>
                            <input type="number" id="mobile_no" maxlength="10" placeholder="Search by mobile number"
                                class="form-control" name="mobile_no">
                        </div>
                        <div class="col-4 mb-4">
                            <label for="email_address" class="form-label">Student E-Mail</label>
                            <input type="email" id="email_address" placeholder="Search by email" class="form-control"
                                name="email">
                        </div>
                        <div class="col-4 d-flex align-items-center">
                            <a onclick="filterStudentDetails()" class="btn btn-primary btn-sm"
                                style="margin-right: 10px;"><i class="bi bi-filter-left"></i> Filter</a>
                            <a href="javascript:void(0)" onclick="reload()" class="btn btn-secondary btn-sm"><i
    ...
    <script>
                ...
                success: function(response) {
                    ...
                    $.each(response.data, function(index, student) {
                        ...
                        if (student.student_details.image != null) {
                            StudentImage = "<img src=" + student.student_details.image +
                                " class='img_fluid' style='height:80px;' />";
                        } else {
                            StudentImage =
                                "<img src='/assets/admin/img/student.png' class='img_fluid' style='height:80px;' />";
                        }
                        table.row.add([
                            index + 1,
                            StudentImage,
                            student.student_details.student_name,
                            new Date(student.student_details.date_of_birth).toLocaleDateString(
                                'en-UK', {
                                    year: 'numeric',
                                    month: '2-digit',
                                    day: '2-digit'
                                }),
                            student.student_class.class_name + '<br>' + student.student_section
                            .section_name,
                            ...
                            '<a class="btn btn-primary btn-sm rounded-pill" href="javascript:void(0)" onclick="EditStudent(\'' +
                            student.student_details.id +
                            '\')">Edit <i class="bi bi-pencil-square"></i> </a><a class="btn btn-success btn-sm rounded-pill" href="javascript:void(0)" onclick="ViewStudent(\'' +
                            student.student_details.id +
                            '\')">View <i class="bi bi-eye-fill"></i></a>' +

                            '<a class="btn btn-secondary btn-sm rounded-pill" href="javascript:void(0)" onclick="StudentPayment(' +
                            student.student_session.id + ',' + student.student_class.id + ',' +
                            student.student_section.id + ',' + student.student_details.id +
                            ')">Fees <i class="bi bi-currency-rupee"></i></a> ' +
                            '<a class="btn btn-danger btn-sm rounded-pill show_confirm" href="javascript:void(0)" onclick="deleteStudent(\'' +
                            student.student_details.id +
        ...
        function StudentPayment(session_id, class_id, section_id, student_id) {
            window.location.href = "fees-payment?session_id=" + session_id + '&class_id=' + class_id + '&section_id=' +

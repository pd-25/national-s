...
@section('content')
                    ...
                    <div class="row my-3">
                        ...
                        <div class="col-4 mb-2">
                            ...
                            <select name="section_id" id="section_id" class="form-select"
                                onchange="getStudent(); GetFeeSettingsData()" required>
                        ...
                        <div class="col-6 mb-2">
                            ...
                            <select name="user_id" id="student_id_bind" class="form-select js-example-basic-single"
                                onchange="GetPaymentHistoryOfStudent();GetStudentFeeSettingsData();" required>
                        ...
                        <div class="col-3 mb-2">
                            ...
                            <select name="month" id="month" class="form-select"
                                onchange="GetFeeSettingsData();GetStudentFeeSettingsData()">
                                <option value="" required>--Select Month--</option>
                                @if (!@empty(GetallMonths()))
                                    @foreach (GetallMonths() as $item)
                                        <option value="{{ @$item }}" {{ $item == date('F') ? 'selected' : '' }}>
                                            {{ @$item }}</option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="col-12 my-2">
                            ...
                            <div class="table-wrapper" style="max-height: 250px; overflow-y: auto;">
                                <table class="table table-bordered table-striped table-sm table-fixed-header"
                                    id="DataTables">
                                    <thead>
                                        ...
                                        @if (@$item->parent_id == null)
                                            ...
                                            <div class="col-4 border">
                                                <div class="input-group my-1">
                                                    ...
                                                    <input type="number" min="0" name="amount[{{ @$item->id }}]"
                                                        onblur="CountTotalPayment()" id="amount_{{ @$item->id }}"
                                                        class="form-control">
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        @if (@$item->parent_id)
                                            @if (@$item->parent->id == @$item->parent_id)
                                                <div class="col-4 mt-3">
                                                    <textarea name="comments[{{ @$item->id }}]" class="form-control" id="comments_{{ @$item->id }}"
                                                        placeholder="{{ @$item->parent->name }} Comments" rows="2"></textarea>
                                                </div>
                                            @endif
                                        @endif
                                    @endforeach
                        ...
                        <div class="col-4">
                            <div class="input-group mb-3">
                                ...
                                <input type="text" min="0" name="total_payable" readonly id="total_payable"
                                    class="form-control">
                            </div>
                        </div>
                        <hr>
                        <div class="col-4 mb-2">
                            <label for="" class="form-label">Payment Mode<span
                                    class="text-danger">*</span></label>
                            <select name="payment_mode" id="payment_mode" onclick="checkPaymentMode(value)"
                        ...
                        <div class="col-12 mt-4">
                            <button type="submit" class="btn btn-success" id="SaveButton">Pay Now</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var fullMonthName = currentDate.toLocaleString('default', {
                month: 'long'
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response, function(index, deposite) {
                            ...
                            table.row.add([
                                index + 1,
                                deposite.student_name + '<br>' + deposite.student_details
                                .admission_number,
                                '<b>' + deposite.payment_number + '</b>',
                                deposite.month + ' / ' + deposite.year,
                                status,
                                "<span class='fw-bold text-danger'>" + deposite.total_payable +
                                "</span>",
                        ...
                        if (!response || response.length === 0) {
                            ...
                            $.each(response, function(index, feesetting) {
                                ...
                                $.each(chargesAmount, function(i, chargeAmount) {
                                    ...
                                    if (monthsValidation[i] && monthsValidation[i].includes(
                                            month)) {
                                        $(inputSelector).val('');
